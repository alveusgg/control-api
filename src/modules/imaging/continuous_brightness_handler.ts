	...
	handle: () => {
		return createFactory<constants.Env>().createHandlers(async (ctx) => {
			...
			let url = VAPIXManager.URLBuilder("com/ptz", camera.host, {
				continuousbrightnessmove: cbrightness.value,
			});
...
export default CBrightnessHandler;
